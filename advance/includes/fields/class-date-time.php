...
class WPForms_Field_Date_Time extends WPForms_Field
{
    ...
    public function field_options($field)
    {
        ...
        foreach ($date_formats as $key => $value) {
            if (in_array($key, array('m/d/Y', 'd/m/Y', 'Y/m/d'), true)) {
        ...
        $time_format = !empty($field['time_format']) ? esc_attr($field['time_format']) : 'H:i';
        $time_formats = array(
            'g:i A' => '12 H',
            'H:i' => '24 H',
        );
        ...
        $time_intervals = array(
            '15' => __('15 minutes', 'wpforms'),
            '30' => __('30 minute', 'wpforms'),
            '60' => __('1 hour', 'wpforms'),
        );
        ...
        echo '</div>';
        $time_minimum = !empty($field['time_minimum']) ? esc_attr($field['time_minimum']) : '';
        $time_maximum = !empty($field['time_maximum']) ? esc_attr($field['time_maximum']) : '';

        echo '</div>';
        echo '</div>';
        echo '<div class="wpforms-field-option-row range_time_block">';
        echo '<div class="range_time">';
        printf('<input type="number" class="placeholder range_time_input" min="0" id="wpforms-field-option-%d-time_minimum" name="fields[%d][time_minimum]" value="%s">', $field['id'], $field['id'], $time_minimum);
        printf('<label for="wpforms-field-option-%d-time_minimum" class="sub-label">%s</label>', $field['id'], __('Minimum time', 'wpforms'));
        echo '</div>';
        echo '<div class="range_time">';
        printf('<input type="number" class="placeholder range_time_input" max="24" id="wpforms-field-option-%d-time_maximum" name="fields[%d][time_maximum]" value="%s">', $field['id'], $field['id'], $time_maximum);
        printf('<label for="wpforms-field-option-%d-time_maximum" class="sub-label">%s</label>', $field['id'], __('Maximum time', 'wpforms'));
        echo '</div>';
        echo '</div>';


        $this->field_option('toggle_active_time', $field);

        $this->defaults = array(
            1 => array(
                'label' => __('11時', 'wpforms'),
                'value' => '11時',
                'default' => '',
            ),
            2 => array(
                'label' => __('13時', 'wpforms'),
                'value' => '13時',
                'default' => '',
            )
        );
        $this->field_option('time_choices', $field);
    ...
    public function field_preview($field)
    {
        ...
        if ('mm/dd/yyyy' === $date_format || 'm/d/Y' === $date_format) {
            $date_first_select = '月';
            $date_second_select = '日';
        } else {
            $date_first_select = '日';
            $date_second_select = '月';
        }
        ...
        if ($date_format === 'Y/m/d') {
            ...
            echo '<select disabled><option>年</option></select>';
            echo '<span>/</span>';
    ...
    public function field_display($field, $field_atts, $form_data)
    {
        ...
        $date_type = !empty($field['date_type']) ? esc_attr($field['date_type']) : 'datepicker';
        $time_placeholder = !empty($field['time_placeholder']) ? esc_attr($field['time_placeholder']) : '';
        $time_minimum = !empty($field['time_minimum']) ? esc_attr($field['time_minimum']) : 0;
        $time_maximum = !empty($field['time_maximum']) ? esc_attr($field['time_maximum']) : 24;
        $time_format = !empty($field['time_format']) ? esc_attr($field['time_format']) : 'g:i A';
        $time_interval = !empty($field['time_interval']) ? esc_attr($field['time_interval']) : '30';
        $form_id = $form_data['id'];
        $day_from_now = !empty($field['limit_character']) ? $field['limit_character'] : null;
        $choices = $field['choices'];
        $is_custom_time = $field['toggle_active'];
        $time_format = 'H:i';

        $time_choices = null;
        if ($choices && $is_custom_time) {
...
// Primary select field.
            printf(
                '<select name="wpforms[fields][%d][time]" id="%s" class="%s" %s %s>',
                $field['id'],
                $field_id,
                $field_class,
                $field_required,
...
// Build the select options.
        ...
        if ('date-time' === $field_format) :
            ...
            if ('dropdown' === $date_type) {
                ...
                printf(
                    '<input type="text" name="wpforms[fields][%d][date]" id="%s" class="%s" placeholder="%s"  data-date-format="%s" %s day_from_now="%s">',
                    $field['id'],
            ...
            if ($choices && $is_custom_time) {
                ...
                printf(
                    '<input type="text" name="wpforms[fields][%d][time]" id="%s" class="%s wpforms-field-date-time-time wpforms-timepicker" placeholder="%s" data-rule-%s="true" data-time-format="%s" data-step="%s" %s minimum-time="%s" maximum-time="%s">',
                    $field['id'],
        ...
        elseif ('date' === $field_format) :
            ...
            if ('dropdown' === $date_type) {
                ...
                printf(
                    '<input type="text" name="wpforms[fields][%d][date]" id="%s" class="%s wpforms-field-date-time-date wpforms-datepicker" placeholder="%s" data-date-format="%s" %s day_from_now="%s">',
                    $field['id'],
                    $field_id,
                    $field_class,
                    $date_placeholder,
    ...
    function field_display_date_dropdowns($format, $field, $field_required, $form_id)
    {
        ...
        if ('Y/m/d' === $format) {
            ...
            echo '<option class="placeholder" selected disabled>年</option>';
            ...
            echo '<option class="placeholder" selected disabled>月</option>';
            for ($i = 1; $i <= 12; $i++) {
                printf('<option value="%d">%d</option>', $i, $i);
            }
            echo '</select>';
            ...
            echo '<option class="placeholder" selected disabled>日</option>';
            for ($i = 1; $i <= 31; $i++) {
                printf('<option value="%d">%d</option>', $i, $i);
            }
            echo '</select>';

        } else if ('m/d/Y' === $format) {
    ...
    public function validate($field_id, $field_submit, $form_data)
    {
        ...
        if (!empty($form_data['fields'][$field_id]['required'])) {
            ...
            if (!empty($form_data['fields'][$field_id]['date_type']) &&
            ...
            if (is_array($field_submit['date'])) {
                ...
                if (!empty($field_submit['date']['m']) &&
                    ...
                    if (('dd/mm/yyyy' === $date_format || 'd/m/Y' === $date_format)) {
                        ...
                        $date = $field_submit['date']['y'] . '/' . $field_submit['date']['m'] . '/' . $field_submit['date']['d'];
                    } else {
