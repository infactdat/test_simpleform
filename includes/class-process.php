...
class WPForms_Process
{
        ...
        foreach ($notifications as $notification_id => $notification) {
            ...
            foreach ($fields as $field) {
                ...
                if ($field['type'] == 'store') {
                    ...
                    if ($value) {
                        $email_encode = wpforms_get_string_between($value, '{', '}');
                        $field['value_raw'] = str_replace('{' . $email_encode . '}', '', $value);
                        $field['value'] = str_replace('{' . $email_encode . '}', '', $value);
                        $email_decode = base64_decode_maybe($email_encode);
                        if (is_email($email_decode)) {
                            $email_decoded = $email_decode;
                        }
                    }
            ...
            if ($email_decoded != '') {
                array_push($email['address'], $email_decoded);
            }
            ...
            // Check if exist previous url

            if ($form_data['settings']['previous_link_enable'] == true) {
                if ($notification['email'] === '{admin_email}') {
                    $previous_link = (!empty($entry['previous_url']) ? $entry['previous_url'] : '');
                    if (isset($previous_link) && $previous_link != '') {
                        $previous_link_content = "\r\n";
                        $previous_link_content .= wpforms_decode_string(__('Previous url', '')) . ":";
                        $previous_link_content .= wpforms_decode_string($previous_link) . "\r\n\r\n";
                        $email['message'] .= $previous_link_content;
                    }
                }
            }
            $email = apply_filters('wpforms_entry_email_atts', $email, $fields, $entry, $form_data, $notification_id);

            if ($form_data['settings']['enable_attachment'] == false) {
                if ($notification['email'] === '{admin_email}') {
                    foreach ($fields as $key => $value) {
                        foreach ($value as $item) {
                            if ($item === 'file-upload' && (strpos($item, 'file-upload') !== false)) {
                                unset($fields[$key]);
                            }
                        }
                    }
            ...
            // Maybe include CC.
            if (!empty($notification['carboncopy']) && wpforms_setting('email-carbon-copy', true)) {
                $emails->__set('cc', $notification['carboncopy']);
            }

            // Maybe include BCC.
            if (!empty($notification['blindcarboncopy']) && wpforms_setting('email-blind-carbon-copy', true)) {
                $emails->__set('bcc', $notification['blindcarboncopy']);
            }
            // Go.
    ...
    public function entry_save($fields, $entry, $form_id, $form_data = '')
    {
        ...
        if ($form_data['settings']['enable_archive'] == true) {
