...
function wpforms_panel_field($option, $panel, $field, $form_data, $label, $args = array(), $echo = true)
{
    ...
    switch ($option) {
        ...
        case 'color':
            $type = !empty($args['type']) ? esc_attr($args['type']) : 'text';
            $output = sprintf(
                '<input type="%s" id="wpforms-panel-field-%s-%s" name="%s" value="%s" placeholder="%s" class="infact_wpforms-color-picker %s" %s>',
                $type,
                sanitize_html_class($panel_id),
                sanitize_html_class($field),
                $field_name,
    ...
    if (!in_array($option, array('checkbox'), true) && !empty($label)) {
        ...
        if (!empty($args['smarttags'])) {
            ...
            $field_label .= '<a href="#" class="toggle-smart-tag-display" data-type="' . $type . '" data-fields="' . $fields . '"><i class="fa fa-tags"></i> <span>' . __('', 'wpforms') . '</span></a>';
        }
        $field_label .= '</label>';
    } else {
        $field_label = '';
