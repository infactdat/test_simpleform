...
class WPForms_Field_Radio extends WPForms_Field
{
    ...
    public function field_options($field)
    {
        ...
        if (!empty($field['show_values']) || apply_filters('wpforms_fields_show_options_setting', false)) {
            $show_values = $this->field_element(
                ...
                array(
                    'slug' => 'show_values',
                    'value' => isset($field['show_values']) ? $field['show_values'] : 1,
    ...
    public function checkAvailableTicket($form_id , $field_id , $field_value, $limit = 10)
    {
        global $wpdb;
        $exist_ticket = $wpdb->get_results("SELECT COUNT(*) FROM  wp_wpforms_entry_fields WHERE  form_id={$form_id} AND field_id={$field_id}  AND value='{$field_value}'", ARRAY_A);

        $exist_ticket = absint($exist_ticket[0]['COUNT(*)']);

        return $exist_ticket >= $limit ? true : false;
    }
    ...
    public function field_display($field, $field_atts, $form_data)
    {
        ...
        foreach ($choices as $key => $choice) {
            ...
            $limit = $choice['limit'] ? $choice['limit'] : 99999;
            printf('<li class="choice-%d depth-%d">', $key, $depth);

            printf('<input type="radio" id="wpforms-%s-field_%d_%d" name="wpforms[fields][%d]" value="%s" %s %s %s>',
                $form_id,
                $field['id'],
                $key,
                $field['id'],
                ...
                $field_required,
                $this->checkAvailableTicket($form_data['id'], $field['id'], $choice['label'], $limit) ? 'disabled' : ''
            );


            printf('<label class="wpforms-field-label-inline" for="wpforms-%d-field_%d_%d">%s%s</label>', $form_id, $field['id'], $key, $this->checkAvailableTicket($form_data['id'], $field['id'], $choice['label'], $limit) ? '【満了】' : '',wp_kses_post($choice['label']));


            if ($val == apply_filters('other_choice_text', 'その他')) {
                printf('<input type="text" name="wpforms[fields][%d]" class="hidden other_choice" value="%s" disabled>',
                    $field['id'],
                    $val
