	...
	public function field_option( $option, $field, $args = array(), $echo = true ) {
		...
		switch ( $option ) {
			...
			case 'limit_character':
				$value   = ! empty( $field['limit_character'] ) ? esc_attr( $field['limit_character'] ) : '';
				$tooltip = __( '文字数を制限したい場合、半角で数字を入力してください。', 'wpforms' );
				$output  = $this->field_element( 'label', $field, array(
					'slug'    => 'limit_character',
					'value'   => __( 'Limit character', 'wpforms' ),
					'tooltip' => $tooltip,
					'type'    => 'number'
				), false );
				$output  .= $this->field_element( 'text', $field, array(
			...
			case 'custom_expression':
				$show_values = $this->field_element(
					...
					$field,
					array(
						'slug'  => 'enable_full_size',
						'value' => isset( $field['enable_full_size'] ) ? $field['enable_full_size'] : '0',
						'desc'  => __( 'Accept full size characters', 'wpforms' ),
					),
					false
				);
				$output      = $this->field_element( 'row', $field, array(
				...
				$show_values = $this->field_element(
					...
					$field,
					array(
						'slug'  => 'enable_half_size',
						'value' => isset( $field['enable_full_size'] ) ? $field['enable_full_size'] : '0',
						'desc'  => __( 'Accept half size characters', 'wpforms' ),
					),
					false
				);
				$output      = $this->field_element( 'row', $field, array(
					'slug'    => 'enable_half_size',
					'content' => $output . $show_values,
				) );
				...
				$show_values = $this->field_element(
					...
					$field,
					array(
						'slug'  => 'enable_english_number',
						'value' => isset( $field['enable_english_number'] ) ? $field['enable_english_number'] : '0',
						'desc'  => __( 'Accept English numbers ', 'wpforms' ),
					),
					false
				);
				$output      = $this->field_element( 'row', $field, array(
				...
				$show_values = $this->field_element(
					...
					$field,
					array(
						'slug'  => 'enable_english_character',
						'value' => isset( $field['enable_english_character'] ) ? $field['enable_english_character'] : '0',
						'desc'  => __( 'Accept English characters ', 'wpforms' ),
					),
					false
				);
				$output      = $this->field_element( 'row', $field, array(
			...
			case 'required':
				...
				$value   = isset( $field['custom_required_text'] ) ? $field['custom_required_text'] : $default;

				$show_values = $this->field_element(
					'text',
					$field,
					array(
						'slug'        => 'custom_required_text',
						'value'       => $value,
						'placeholder' => __( 'Enter any of text you want to custom for required field ', 'wpforms' )
					),
					false
				);
				$output      = $this->field_element( 'row', $field, array(
			...
			case 'choices':
				$tooltip = __( 'Add choices for the form field. If choice is Other choice or その他, an input will be generated', 'wpforms' );
			...
			case 'default_value':
				...
				$output = $this->field_element( 'label', $field, array(
					'slug'    => 'default_value',
					'value'   => __( 'Default Value', 'wpforms' ),
					'tooltip' => $tooltip,
				), false );
				$output .= $this->field_element( 'text', $field, array(
					'slug'  => 'default_value',
			...
			case 'placeholder':
				...
				$output  = $this->field_element( 'label', $field, array(
					'slug'    => 'placeholder',
					'value'   => __( 'Placeholder Text', 'wpforms' ),
					'tooltip' => $tooltip
				), false );
				$output  .= $this->field_element( 'text', $field, array(
					'slug'  => 'placeholder',
			...
			case 'sublabel_hide':
				...
				$output = $this->field_element( 'checkbox', $field, array(
					'slug'    => 'sublabel_hide',
					'value'   => $value,
					'desc'    => __( 'Hide Sub-Labels', 'wpforms' ),
					'tooltip' => $tooltip
				), false );
				$output = $this->field_element( 'row', $field, array(
					'slug'    => 'sublabel_hide',
			...
			case 'input_columns':
				...
				$output  = $this->field_element( 'label', $field, array(
					'slug'    => 'input_columns',
					'value'   => __( 'Choice Layout', 'wpforms' ),
					'tooltip' => $tooltip
				), false );
				$output  .= $this->field_element( 'select', $field, array(
					'slug'    => 'input_columns',
			...
			case 'dynamic_choices':
				...
				$output  = $this->field_element( 'label', $field, array(
					...
					'value'   => __( 'Dynamic Choices', 'wpforms' ),
					'tooltip' => $tooltip
				), false );
				$output  .= $this->field_element( 'select', $field, array(
					'slug'    => 'dynamic_choices',
