 ...
 * @since      1.0.0
 */
class WPForms_Field_Store extends WPForms_Field {
	...
	public function init() {
		...
		$this->name     = __( 'List of store', 'wpforms' );
		$this->type     = 'store';
		$this->icon     = 'fa-list-ul';
		$this->order    = 11;
		$this->defaults = array(
			011 => array(
				'label'   => __( 'Store 1', 'wpforms' ),
				'value'   => '',
				'default' => '',
			),
			022 => array(
				'label'   => __( 'Store 2', 'wpforms' ),
				'value'   => '',
				'default' => '',
			),
	...
	public function field_display( $field, $field_atts, $form_data ) {
		...
		$field          = apply_filters( 'wpforms_store_field_display', $field, $field_atts, $form_data );
		...
		foreach ( $choices as $key => $choice ) {
			...
			$email = wpforms_get_string_between( $label, '{', '}' );

			$email_decoded = base64_encode( $email );

			$label_filtered = ( ! empty( $email ) ) ? str_replace( '{' . $email . '}', '', $label ) : $label;

			$value_filtered = str_replace( $email, $email_decoded, $label );
...
//            $val      = isset( $field['show_values'] ) ? esc_attr( $choice['value'] ) : esc_attr( $choice['label'] );
			...
			printf( '<label class="wpforms-field-label-inline" for="wpforms-%d-field_%d_%d">%s</label>', $form_id, $field['id'], $key, wp_kses_post( $label_filtered ) );
