...
class WPForms_Frontend
{
    ...
    private $primary_color = '#f0394d';

    private $primary_text_color = '#fff';
    ...
    public function __construct()
    {
        ...
        add_action('wpforms_display_field_before', array($this, 'wrap_element_before'), 10);
        add_action('wpforms_display_field_before', array($this, 'field_label'), 15, 2);
        add_action('wpforms_display_field_after', array($this, 'wrap_element_after'), 20);
        add_action('wpforms_display_field_before', array($this, 'wrap_element_before'), 25);
        add_action('wpforms_display_field_before', array($this, 'field_container_open'), 30, 2);
        add_action('wpforms_display_field_before', array($this, 'field_description'), 35, 2);
        add_action('wpforms_display_field_after', array($this, 'field_error'), 5, 2);
        add_action('wpforms_display_field_after', array($this, 'field_description'), 10, 2);
    ...
    public function fields($form_data, $deprecated, $title, $description, $errors)
    {
        ...
        if (wpforms_setting('primary-color')) {
            $this->primary_color = wpforms_setting('primary-color');

            $this->primary_text_color = '#fadas';
        }


        // Form fields area.
        echo '<div class="wpforms-field-container">';
        if (isset($form_data['settings']['custom_content_editor'])) {
            echo '<div class="wpforms_custom_content">';
            echo $form_data['settings']['custom_content_editor'];
            echo '</div>';
        }
        ...
        echo '<table class="table_wrap_fields">';
    ...
    public function wrap_element_before()
    {
        echo '<td>';
    }


    public function wrap_element_after()
    {
        echo '</td>';
    }
    ...
    function get_field_properties($field, $form_data, $attributes = array())
    {
        ...
        if (isset($field['limit_character'])) {
            if ($field['limit_character']) {
                array_push($attributes['field_class'], 'limit_character');
                array_push($attributes['field_class'], 'limit_number_' . $field['limit_character']);
            }
        }


        if (isset($field['enable_full_size'])) {
            if ($field['enable_full_size']) {
                array_push($attributes['field_class'], 'enable_full_size');
            }
        }
        if (isset($field['enable_half_size'])) {
            if ($field['enable_half_size']) {
                array_push($attributes['field_class'], 'enable_half_size');
            }
        }

        if (isset($field['enable_english_number'])) {
            if ($field['enable_english_number']) {
                array_push($attributes['field_class'], 'enable_number');
            }
        }
        if (isset($field['enable_english_character'])) {
            if ($field['enable_english_character']) {
                array_push($attributes['field_class'], 'enable_english_character');
            }
        }
    ...
    public function field_label($field, $form_data)
    {
        ...
        if (isset($field['custom_required_text'])) {
            $custom_required_text = $field['custom_required_text'];
        } else {
            $custom_required_text = '';
        }

        $required = $label['required'] ? apply_filters('wpforms_field_required_label', ' <span class="wpforms-required-label">' . ($custom_required_text != '' ? $custom_required_text : '*') . '</span>') : '';
    ...
    public function field_description($field, $form_data)
    {
        ...
        // If it is radio type so we move description to top.
    ...
    public function honeypot($form_data, $deprecated, $title, $description, $errors)
    {
        ...
        echo '<input type="text" name="wpforms[hp]" id="wpforms-field_hp" >';
    ...
    public function foot($form_data, $deprecated, $title, $description, $errors)
    {
        ...
        if (isset($_SERVER['HTTP_REFERER'])) {
            echo '<input type="hidden" name="wpforms[previous_url]" value="' . $_SERVER['HTTP_REFERER'] . '">';
        }
    ...
    public function assets_js()
    {
        ...
        wp_enqueue_script(
            'generate_address',
            WPFORMS_PLUGIN_URL . 'assets/js/ajaxzip3.js',
            array('jquery'),
            '1.15.1',
            true
        );
        ...
        wp_enqueue_script(
            'addtional-validate',
            WPFORMS_PLUGIN_URL . 'assets/js/addtional-validate.js',
            array('jquery'),
            WPFORMS_VERSION,
            true
        );
        ...
        global $infactOptions;
        $secret_key = $infactOptions['recaptcha-secret-key'];
        $site_key = $infactOptions['recaptcha-site-key'];
        if (empty($site_key)) {
            $site_key = wpforms_setting('recaptcha-site-key');
        }
